...
def train_model():
    ...
    # Entraînement du modèle pour les statistiques défensives
    model_defensive = RandomForestRegressor()
    model_defensive.fit(X_defensive, y_defensive)

    # Validation du modèle défensif
    ...
    # Sauvegarde du modèle défensif
    joblib.dump(model_defensive, 'prediction_model_defensive.pkl')

    # Entraînement du modèle pour les statistiques offensives
    model_offensive = RandomForestRegressor()
    model_offensive.fit(X_offensive, y_offensive)

    # Validation du modèle offensif
    predictions_offensive = model_offensive.predict(X_test_offensive)
    mse_offensive = mean_squared_error(y_test_offensive, predictions_offensive)
    print("Mean Squared Error (Offensive):", mse_offensive)

    # Sauvegarde du modèle offensif
    joblib.dump(model_offensive, 'prediction_model_offensive.pkl')

    # Entraînement du modèle pour les statistiques générales
    model_general = RandomForestRegressor()
    model_general.fit(X_general, y_general)
    ...
    # Sauvegarde du modèle général
    joblib.dump(model_general, 'prediction_model_general.pkl')

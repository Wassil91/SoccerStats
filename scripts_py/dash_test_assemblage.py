...
def test_all_dash():
    ...
    def dash_L1():
        ...
        def display_page(pathname):
            if pathname.startswith == '/L1_Liga_Dash':
                return l1_liga_dash_layout()
            else:
                return render_home_page()

                # Définissez la mise en page pour la nouvelle page "L1_Liga_Dash"
        def l1_liga_dash_layout():
            # Charger les données de la Ligue 1 et de la Liga
            df = load_data("matchs_L1")
            df_liga = load_data_liga("matchs_Liga")
            df_liga['Championnat'] = 'Liga'
            df = pd.concat([df, df_liga], ignore_index=True)

            # Créer un histogramme pour le nombre de buts par match
            fig = px.histogram(df, x='TotalButs', nbins=20, color='Championnat', 
                            labels={'TotalButs': 'Nombre total de buts', 'count': 'Nombre de matchs'},
                            title='Histogramme du nombre de buts dans la Ligue 1 et la Liga',
            ...
            fig.update_layout(
                ...
                legend_title='Championnat',
                bargap=0.2,
                barmode='overlay'
            )

            return html.Div([
                html.H2('Histogramme du nombre de buts dans la Ligue 1 et la Liga', style={'color': colors['text'], 'padding': '10px'}),
                dcc.Graph(figure=fig)
            ], style={'margin': '20px', 'borderRadius': '10px', 'backgroundColor': colors['background'], 'padding': '20px'})
        ...
        def update_graphs(championship, selected_season):
            ...
            histogram_figure = px.histogram(df, x='Score', title='Distribution des scores', labels={'Score': 'Buts'}, color='Championnat')
            histogram_figure.update_traces(marker_color=l1_color, selector={'name': 'Ligue 1'})
            histogram_figure.update_traces(marker_color=liga_color, selector={'name': 'Liga'})
            if championship == 'L1':
                # Filtrer les données de la Liga
                df_liga = load_data_liga("matchs_Liga")
                df_liga['Championnat'] = 'Liga'
                # Ajouter les données de la Liga à la figure
                histogram_figure.add_trace(
                    go.Histogram(x=df_liga['Score'], name='Liga', marker_color=liga_color)
                )

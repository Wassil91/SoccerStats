...
def calculate_summary_statistics_PL(df):
...
def dash_PL():
    ...
    app.layout = html.Div(style={'backgroundColor': colors['background'], 'fontFamily': 'Arial'}, children=[
        ...
        html.H1(
            children='Dashboard des différentes saisons de la Liga',
            style={
                'textAlign': 'center',
                'color': colors['text'],
                'padding': '20px'
    ...
    def update_graphs(selected_season):
        if selected_season == '21_22':
            df = load_data_PL("matchs_PL_21-22")
        elif selected_season == '22_23':
            df = load_data_PL("matchs_PL_22-23")
        elif selected_season == '20_21':
            df = load_data_PL("matchs_PL_20-21")
        else:
            df = load_data_PL("matchs_PL")  # For current season

        goals_summary, goals_conceded_summary, goals_per_matchday = calculate_summary_statistics_PL(df)

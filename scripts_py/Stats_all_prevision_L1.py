...
collection_name = "Resultats_Predits"
...
saisons = ["matchs_L1", "matchs_L1_22-23", "matchs_L1_21-22", "matchs_L1_20-21"]

# Récupérer les moyennes des statistiques offensives pour chaque équipe
stats_off = {}
for document in db["Stats_off_prevision_L1"].find():
...
stats_def = {}
for document in db["Stats_def_prevision_L1"].find():
    equipe = document["Équipe"]
    del document["_id"]
    del document["Équipe"]
...
stats_gen = {}
for document in db["Stats_gen_prevision_L1"].find():
    equipe = document["Équipe"]
    del document["_id"]
    del document["Équipe"]
...
for saison in saisons:
    ...
    for match in matchs:
        ...
        # Ajuster la possession en fonction du nombre de tirs par matchs
        tirs_pm_dom = stats_dom.get("Tirs pm", 0)
        tirs_pm_ext = stats_ext.get("Tirs pm", 0)
        total_tirs_pm = tirs_pm_dom + tirs_pm_ext

        # Vérifier si la somme des tirs par matchs est non nulle pour ajuster la possession
        if total_tirs_pm > 0:
            # Calculer la proportion de possession en fonction du nombre de tirs par matchs

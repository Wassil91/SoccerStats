...
def calculate_summary_statistics_bundes(df):
...
def dash_bundes():
    ...
    app.layout = html.Div(style={'backgroundColor': colors['background'], 'fontFamily': 'Arial'}, children=[
        ...
        html.H1(
            children='Dashboard des différentes saisons de la bundesliga',
            style={
                'textAlign': 'center',
                'color': colors['text'],
                'padding': '20px'
    ...
    def update_graphs(selected_season):
        if selected_season == '21_22':
            df = load_data_bundes("matchs_bundesliga_21-22")
            ...
            df = load_data_bundes("matchs_bundesliga")  # For current season

        goals_summary, goals_conceded_summary, goals_per_matchday = calculate_summary_statistics_bundes(df)
...
if __name__ == '__main__':
    dash_bundes().run_server(debug=True)
